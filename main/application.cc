                ...
                if (content_length > 0) {
                    ...
                    if (buffer != NULL) {
                        ...
                        if (json != NULL) {
                            ...
                            if (command && !cJSON_IsNull(command)) {
                                cJSON* stream_url = cJSON_GetObjectItem(command, "stream_url");
                                cJSON* cmd_id = cJSON_GetObjectItem(command, "id");

                                if (stream_url && stream_url->valuestring && cmd_id) {
                                    int command_id = cmd_id->valueint;
                                    ...
                                    ack_config.url = ack_url;
                                    ack_config.method = HTTP_METHOD_POST;

                                    esp_http_client_handle_t ack_client = esp_http_client_init(&ack_config);
                                    if (ack_client != NULL) {
                                        char post_data[128];
                                        snprintf(post_data, sizeof(post_data), "{\"device_id\":\"%s\"}", YT_DEVICE_ID);

                                        esp_http_client_set_header(ack_client, "Content-Type", "application/json");
                                        esp_http_client_set_post_field(ack_client, post_data, strlen(post_data));
...
void Application::HandleNetworkConnectedEvent() {
    ...
    if (!yt_task_created) {
        ...
        xTaskCreate(youtube_music_poll_task, "yt_music_poll", 4096, this, 3, NULL);
    }
}
